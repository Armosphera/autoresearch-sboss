"""
eval.py — fixed eval harness for the chat-client example.

DO NOT MODIFY. The agent edits workflow.py. This file is the judge.

Runs run_workflow() over the eval set, computes field-level F1 over the
{ok, text, model, responseId, provider, error_code, error_status, last_request.method,
last_request.body, last_request.headers} field set.
"""
...
def score_outputs(expected: dict, predicted: dict) -> dict[str, float]:
    per_field = {f: (1.0 if _eq(_get(expected, f), _get(predicted, f)) else 0.0) for f in EVAL_FIELDS}
...
def main() -> int:
    ...
    print("autoresearch-sboss eval [chat-client example]")
    print(f"  eval_set:     {eval_path}")
    print(f"  budget_sec:   {budget_sec}")
    print(f"  llm_endpoint: {os.environ.get('LLM_ENDPOINT_URL', '(no LLM — mock safeFetch from eval_set)')}")
    print()
