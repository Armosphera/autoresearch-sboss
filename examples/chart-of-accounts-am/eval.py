"""
eval.py — fixed eval harness for the chart-of-accounts-am example.

DO NOT MODIFY. The agent edits workflow.py. This file is the judge.

Runs validate_code() (via run_workflow) over the eval set in eval_set.json, computes
field-level exact-match F1 over {code, ok, normalized, hy, class, type, error}.
"""
...
EVAL_FIELDS = ("code", "ok", "normalized", "hy", "class", "type", "error")
...
def main() -> int:
    ...
    print("autoresearch-sboss eval [chart-of-accounts-am example]")
    print(f"  eval_set:     {eval_path}")
    print(f"  data_file:    {workflow.DATA_FILE}")
    print(f"  n_accounts:   {len(workflow.STANDARD_ACCOUNTS)}")
