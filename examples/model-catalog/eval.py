"""
eval.py — fixed eval harness for the model-catalog example.

DO NOT MODIFY. The agent edits workflow.py. This file is the judge.

Per-case field set: {online, source, reason, modelsCount, lastRequestUrl,
lastRequestMethod, lastRequestHeaders} — uses keys in each item's expected.
"""
...
def run_eval(eval_set: list[dict], budget_sec: float) -> tuple[float, dict]:
    ...
    for i, item in enumerate(eval_set):
        ...
        for f in item["expected"]:
            ...
            per_field_counts[f] = per_field_counts.get(f, 0) + 1

    n = len(eval_set)
    overall = (sum(per_item_scores) / n) * 100.0 if n else 0.0
    per_field_pct = {
        f: (v / per_field_counts[f]) * 100.0 if per_field_counts.get(f) else 0.0
...
def main() -> int:
    ...
    print("autoresearch-sboss eval [model-catalog example]")
