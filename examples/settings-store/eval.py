"""
eval.py — fixed eval harness for the settings-store example.

DO NOT MODIFY. The agent edits workflow.py. This file is the judge.

Each test case gets a fresh tmp data dir (created here, cleaned up after).
Per-case field set: {result} only.
"""
...
def run_eval(eval_set: list[dict], budget_sec: float) -> tuple[float, dict]:
    ...
    for i, item in enumerate(eval_set):
        ...
        # Fresh tmp data dir per test case
        data_dir = tempfile.mkdtemp(prefix=f"settings-store-{i:02d}-")
        try:
            # Inject dataDir into input
            item_input = dict(item["input"])
            item_input["dataDir"] = data_dir
            try:
                predicted = workflow.run_workflow(item_input)
...
def main() -> int:
    ...
    print("autoresearch-sboss eval [settings-store example]")
    print(f"  eval_set:     {eval_path}")
    print(f"  budget_sec:   {budget_sec}")
    print(f"  llm_endpoint: {os.environ.get('LLM_ENDPOINT_URL', '(no LLM — file I/O + pure function)')}")
    print()
