"""
eval.py — fixed eval harness for the payroll-ru example.

DO NOT MODIFY. The agent edits workflow.py. This file is the judge.

Per-case field set: uses the keys in each item's `expected` dict as the
fields to score. For scalar-returning operations (ndfl, insurance,
childDeduction), expected = {result: <number>}. For compute_monthly_payroll,
expected = {result: <full dict>}.
"""
...
# Per-case field set — see load_eval_set() for how fields are derived.
...
def load_eval_set(path: Path) -> list[dict]:
    ...
    for i, item in enumerate(data):
        ...
        if not item["expected"]:
            raise ValueError(f"eval item {i} has empty 'expected' — at least one field is required")
    return data
...
def main() -> int:
    ...
    print("autoresearch-sboss eval [payroll-ru example]")
